    ...
    def _copy_to_shared_memory(kwargs, shared_memory):
        ...
        for k in kwargs:
            if isinstance(kwargs[k], BroadcastNumpy):
                ...
                assert len(shared_memory) >= count, "Allocate more shared memory"
                shared_memory[:count] = broadcast_numpy._arr
                del kwargs[k]
                kwargs[BroadcastNumpy._LABEL] = (k, count)
...
def node_parallel(node_signatures, conn, shared_memory):
    ...
    while True:
        ...
        if BroadcastNumpy._LABEL in kwargs:
            name = kwargs[BroadcastNumpy._LABEL][0]
            count = kwargs[BroadcastNumpy._LABEL][1]
            del kwargs[BroadcastNumpy._LABEL]
            kwargs[name] = np.copy(np.frombuffer(shared_memory, dtype=np.float32, count=count))
        node_indices = node_indices or nodes.keys()
        outputs = []
...
class Transport(object):
    def __init__(self, nodes, parallel=False, shared_memory_size=0, number_of_processes=1):
        self._nodes = nodes
        ...
        if self._parallel:
            ...
            self._shared_memory = multiprocessing.Array(
                ctypes.c_float, shared_memory_size, lock=False)
            self._shared_memory_numpy = np.frombuffer(self._shared_memory, dtype=np.float32)
    ...
    def call_nodes_method(self, node_method, **kwargs):
        if not self._parallel:
            ...
            return self._call_nodes_method_parallel(node_method, **kwargs)
    ...
    def _update_from_nodes_stat(self, node_index, node_method, outputs):
        ...
        for output in outputs:
            self._update_nodes_stat(self._stat_from_nodes, node_index, node_method, output,
    ...
    def _call_nodes_method_parallel(self, node_method, **kwargs):
        for node_index in range(self._number_of_nodes):
            self._update_to_nodes_stat(node_index, node_method, kwargs)
        BroadcastNumpy._copy_to_shared_memory(kwargs, self._shared_memory_numpy)
        for group_index in range(self._number_of_processes):
            self._connections[group_index].send([None, node_method, kwargs])
        outputs = [None] * self._number_of_nodes
        self._init_max_stat_current()
        for group_index in range(self._number_of_processes):
            ...
            for (node_index, output) in node_index_outputs:
                ...
                assert outputs[node_index] is None
